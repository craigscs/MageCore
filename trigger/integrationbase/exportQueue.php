...
try {
    $codes = Mage::app()->getRequest()->getParam('export_filter');
    $codes = $codes ? explode(',', $codes) : array();
    $limit = Mage::app()->getRequest()->getParam('max_count');
    $operationId = Mage::app()->getRequest()->getParam('operation_id', 0);

    /** @var $export Vaimo_IntegrationBase_Model_Export_Queue */
    $export = Mage::getModel('integrationbase/export_queue');
    $export->run($codes, $limit, $operationId);
} catch (Exception $e) {
